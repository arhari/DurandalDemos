define(['../system', '../viewModel', '../app'], function (system, viewModel, app) {
    ...
    function shouldStopNavigation() {
        var sammyLastLocation = sammy.last_location[1].replace('/', '');
        return cancelling
            || sammyLastLocation == previousRoute
            || sammyLastLocation == previousRoute + '#/' + navigationDefaultRoute
            || sammyLastLocation + '#/' + navigationDefaultRoute == previousRoute;
    }
    ...
    function ensureExternalRoute() {
        var currentRoute = sammy.last_location[1];
        if (_(currentRoute).startsWith('/') && previousRoute !== undefined) {
            var currentUrl = _(_(currentRoute).splice(0, 1)).strLeft('#');
            var previousUrl = _(previousRoute).strLeft('#');
